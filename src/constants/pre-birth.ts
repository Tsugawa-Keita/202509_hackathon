...
// 優先度タイプのバッジ設定
export const priorityTypeBadges: Record<number, { accent: string; label: string; support: string }> = {
  1: {
...
export const getMotherSummaryByWeeks = (weeksPregnant: number) => {
  if (weeksPregnant >= PREGNANCY_LIMITS.nearDueWeeks) {
    return {
      ...
      support: "こまめな水分補給の声かけや温かい飲み物の用意、夜間のサポート体制を整えておきましょう。",
      title: "出産直前のママの状態",
    };
  }
  ...
  if (weeksPregnant >= PREGNANCY_LIMITS.fullTermWeeks) {
    return {
      ...
      support: "短時間の散歩やストレッチを一緒に行い、日常の家事は積極的に肩代わりしましょう。",
      title: "臨月のママの状態",
    };
  }
  ...
  return {
    ...
    support: "TODOの棚卸しを一緒に行い、優先度の高い準備をあなたが先導しましょう。",
    title: "産前1ヶ月のママの状態",
  };
};
