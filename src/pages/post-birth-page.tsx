...
const getImportantTask = (tasks: TodoItem[] | undefined, completedIdSet: Set<string>) => {
...
const formatProgressLabel = (completed: number, total: number) => `${completed} / ${total} 件`;

const IMPORTANT_TASK_EMPTY_MESSAGE = "未完了のTODOはありません。お疲れさまでした！";
...
const buildImportantTaskContent = ({ anchorId, importantTask, isChecklistReady }: ImportantTaskContentParams) => {
  if (!isChecklistReady) {
    return null;
  }

  if (!importantTask) {
    return <p className="text-slate-600 text-sm">{IMPORTANT_TASK_EMPTY_MESSAGE}</p>;
  }

  const priorityBadge = POST_BIRTH_PRIORITY_BADGES[importantTask.priorityType] ?? POST_BIRTH_PRIORITY_BADGES[1];
...
const buildScheduleContent = ({ isScheduleReady, schedule }: ScheduleContentParams) => {
  if (!isScheduleReady) {
    return null;
  }
...
const PostBirthPage = ({ state }: PostBirthPageProps) => {
  ...
  const todoChecklist = useTodoChecklist({
    ...
    loadError: tasksError ? "TODOの読み込みに失敗しました。時間をおいて再度お試しください。" : "",
    state,
    todos: tasks ?? [],
  });

  const daysAfterBirth = useMemo(() => calculateDaysAfterBirth(state.dueDate), [state.dueDate]);
  ...
  const progressLabel = formatProgressLabel(todoChecklist.completedCount, todoChecklist.totalCount);

  const isChecklistReady = !todoChecklist.isLoading && todoChecklist.loadError === "";
  ...
  return (
    <>
      <Dialog onOpenChange={setCelebrationOpen} open={isCelebrationOpen}>
        <DialogContent aria-describedby="post-birth-celebration" role="alertdialog">
          <DialogHeader>
      ...
      <main className="flex min-h-screen flex-col items-center bg-[#FFF2D4] px-6 py-12 text-slate-900" id="top">
        <Card className="w-full max-w-4xl border-none bg-white shadow-md">
          ...
          <CardContent>
            <p className="text-base text-slate-600 leading-relaxed">{PAGE_CONTENT.description}</p>
          </CardContent>
        </Card>

        <Card className="mt-10 w-full max-w-4xl border-none bg-white shadow-md">
          <CardHeader>
            <CardDescription className="text-rose-500">{SECTION_TITLES.importantTask}</CardDescription>
            <CardTitle className="font-bold text-2xl">{SECTION_DESCRIPTIONS.importantTask}</CardTitle>
          </CardHeader>
          <CardContent>
            {todoChecklist.isLoading ? <p className="text-slate-500 text-sm">TODOを読み込み中です…</p> : null}
        ...
        <Card className="mt-10 w-full max-w-4xl border-none bg-white shadow-md">
          ...
          <CardContent>
            {isScheduleLoading ? <p className="text-slate-500 text-sm">スケジュールを読み込み中です…</p> : null}
            {scheduleError ? (
              <p className="text-rose-600 text-sm" role="alert">
